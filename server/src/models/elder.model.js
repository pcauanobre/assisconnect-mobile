const Joi = require("joi"); // joi utilizaod para validação

class PessoaIdosaModel {
    // classe do modelo
    constructor(data = {}) {
        // validação com schema Joi
        const { error, value } = ElderModel.schema.validate(data, {
            abortEarly: false,
        });
    ...
    static get schema() {
        return Joi.object({
            ...
            createdAt: Joi.date()
                .timestamp()
                .default(() => new Date(), "tempo atual"), // esse aqui sempre vai usar o tempo atual
        });
    }
}

module.exports = PessoaIdosaModel; // export
